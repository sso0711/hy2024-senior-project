...
//20170308
//修改成多线程，之后不显示时间
public class WRNetDisSizThread extends Thread {

	public WRNetDisSizThread() {
	...
	public void run(){
		...
		n: for (int i = 0; i < 50000; i++) {
			String path = "http://192.168.1.107" + ":8181/onos/v1/topology";
			...
			if (responseCode == 200) {
				...
				System.out.println("Answer form server:" + result);
				JSONObject Topojson = new JSONObject(result);
				System.out.println("JSON:" + Topojson);
			...
			if (i >= 2) {
				...
				if (devices[i].equals(devices[i - 1]) && devices[i].equals(devices[i - 2])) {
					jishu = jishu + 1;
					switch (jishu) {
					case 1:
						try {
							System.out.println("first check，device number: " + devices[i]);
					...
					case 2:
						try {
							System.out.println("second check，device number: " + devices[i]);
					...
					case 3:
						...
						try {
							System.out.println("third check，device number: " + devices[i]);
