...
public class ReactivePathProvisioningRate {
	...
	public static void go() {
		...
		Log.exportLog(ReactivePathProvisioningRate.class.getSimpleName(), 6);
	}
	...
	private static void start_task() {
		try {// for sleep
			...
			if (Global.PROVISINIONING_PACKET_TYPE == Global.ARP_REPLY) {
				...
				sendPacket(Global.dstHosts[0].getConnectedNode(), dstOFPacketIn);
				...
				sendPacket(Global.srcHosts[0].getConnectedNode(), OFPacketIn);
	...
	private static void sendPacket(Node node, ArrayList<OFMessage> of) {
		ArrayList<ArrayList<OFMessage>> l = new ArrayList<ArrayList<OFMessage>>();
		try {
			...
			n: while (true) {
				...
				while (msgList.size() != Global.BULK_SIZE) {
					if (Global.BUFF_INDEX == of.size()) {
						break n;
					} else {
						msgList.add(of.get(Global.BUFF_INDEX++));
	...
	private static void loss_check(int bulk_size, double inter_arrival_time) {
		// TODO Auto-generated method stub
		ArrayList<Long> ofin = Result.REACTIVE_PATH_PROVISIONING_TIME_LIST_OFIN;
		ArrayList<Long> flowMod = Result.REACTIVE_PATH_PROVISIONING_TIME_LIST_FLOWMOD;
