...
public class S_RunButton {
	...
	private static void checkDistributed() {// 把所有的变数全都放到json格式变量里面
		...
		if (Global.IS_ENABLE_DISTRIBUTED && Global.IS_MASTER) {
			...
			System.out.println("msgType=1 send!");
			...
			System.out.println("IP1:" + IP1 + "   tool_port1:" + tool_port1 + "    server port1:" + server_port1);
			...
			System.out.println("IP2:" + IP2 + "   tool_port2:" + tool_port2 + "    server port2:" + server_port2);
			...
			for (int i = 1; i < listSize; i++) {
				...
				message.put("Tool_IP1", Global.distribudtedTool_IP_List.get((i + listSize - 1) % listSize));
				message.put("Tool_Port1", Global.distribudtedTool_PortList[1 % distributedPortCount]);
				message.put("Tool_Sever_Port1", Global.distribudtedTool_PortList[0 % distributedPortCount]);

				System.out.println("IP1:" + Global.distribudtedTool_IP_List.get((i + listSize - 1) % listSize)
						+ "   tool_port1:" + Global.distribudtedTool_PortList[1 % distributedPortCount]
						+ "    server port1:" + Global.distribudtedTool_PortList[0 % distributedPortCount]);

				message.put("Tool_IP2", Global.distribudtedTool_IP_List.get((i + 1) % listSize));
				message.put("Tool_Port2", Global.distribudtedTool_PortList[0 % distributedPortCount]);
				message.put("Tool_Sever_Port2", Global.distribudtedTool_PortList[1 % distributedPortCount]);

				System.out.println("IP2:" + Global.distribudtedTool_IP_List.get((i + 1) % listSize) + "   tool_port2:"
						+ Global.distribudtedTool_PortList[0 % distributedPortCount] + "    server port2:"
						+ Global.distribudtedTool_PortList[1 % distributedPortCount]);

				DistributedServer.channelMap.get(IP).write(message.toString());
	...
	public static void startTasks() {// 按 run按钮之后的主要实行代码
		...
		// 临时先这么用，有时间再加class
		Main.S_TOPO_TEXT.setText("Topology Type: " + (Global.TOPO_TYPE == 0 ? "Linear" : "Ring") + "\nNumber Of Node: "
				+ Global.NUMBER_OF_TEST_SWITCH);

		Main.S_progressBarTotal.setMaximum(1);// 进度条
		...
		case Global.ASYNCHRONOUS_MESSAGE_PROCESSING_RATE:
			...
			Main.S_progressBarTotal.setValue(Main.S_progressBarTotal.getMaximum());
		...
		case Global.TEST_TOTAL:
			...
			Global.TEST_INIT_TIME = System.currentTimeMillis();
			Main.S_progressBarTotal.setMaximum(6);
			Main.S_progressBarTotal.setValue(0);
