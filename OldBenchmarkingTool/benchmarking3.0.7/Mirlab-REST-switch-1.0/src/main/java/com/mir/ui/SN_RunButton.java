...
import com.mirlab.metric.northbound2.SsyncProactiveProTime_old;
...
public class SN_RunButton {
	...
	public static void startTasks() {// 按 run按钮之后的主要实行代码
		...
		Main.gui.SN_LOG_TEXT.setText("");
		Main.gui.SN_TOPO_TEXT.setText("");
		...
		Main.gui.SN_TOPO_TEXT.append("Topology Type:" + Global.topoType + "\n");

		Main.gui.SN_TOPO_TEXT.append("Number Of Node: " + Global.NUMBER_OF_TEST_SWITCH);
		...
		System.out.println("############## what : " + Global.snboundMetric.toString());
		
		switch (Global.snboundMetric) {
		
		case PROACTIVE_PATH_PROVISIONING:
...
//			pu.updateStart(1, 1);
			...
			AsyncProactiveProvisioningTimeWR.go();
		...
		case TOPOLOGY_DISCOVERY_TIME:// 拓扑发现时间
			...
			Main.gui.SN_progressBar.setMaximum(Global.PROGRESS_MAX_VALUE);
			Main.gui.SN_progressBar.setValue(0);
			Global.TEST_INIT_TIME = System.currentTimeMillis();
			// 获取现在时间并赋值给global变数
...
//			}
		...
		case FORWARDING_TABLE_CAPACITY:
			...
			Main.gui.SN_progressBar.setValue(0);
			Global.TEST_INIT_TIME = System.currentTimeMillis();
			Main.gui.SN_startButton.setEnabled(false);
			Main.gui.SN_metricList.setEnabled(false);
...
//				AsyncFowardTabCapacityWR.go();
		...
		case TEST_TOTAL:
			...
			Main.gui.SN_progressBarTotal.setValue(0);
//			Tasks.START_TASK_15_NEW();
		...
		Main.gui.SN_startButton.setEnabled(true);// run按钮不可按
		Main.gui.SN_metricList.setEnabled(true);// metric 栏不可选择
