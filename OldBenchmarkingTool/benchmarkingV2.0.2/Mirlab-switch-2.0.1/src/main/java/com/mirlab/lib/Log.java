...
public class Log {
	...
	private static void exportLog(String metricName) {
		...
		try {
			...
			for (String value : Main.S_LOG_TEXT.getText().split("\n")) {
				bw.write(value);
				bw.newLine();
			}
	...
	private static void exportLogSummary(String metricName, int metricIndex) {
		...
		try {
			...
			value = dataString + "/" + metricName + "/" + Main.S_comboBoxTopoType.getSelectedItem().toString() + "/"
					+ Global.NUMBER_OF_TEST_SWITCH + "/" + Main.S_RESULT_TABLE.getModel().getValueAt(metricIndex, 2);
			bw.write(value);
			bw.newLine();
		} catch (IOException e) {
			e.printStackTrace();
