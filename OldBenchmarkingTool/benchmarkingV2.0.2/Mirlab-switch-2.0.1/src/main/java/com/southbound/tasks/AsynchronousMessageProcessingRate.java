...
public class AsynchronousMessageProcessingRate {
	...
	private static void start_task() {
		...
		if (Global.UNKNOWN_PACKET_TYPE == Global.ECHO_PACKET) {
			...
			src.getConnectedNode().sendUnknownPacketThroughput_ofMessage(ofpi);
			...
			srcHost.getConnectedNode().sendUnknownPacketThroughput_ofMessage(ofpi);
	...
	private static void sendPacket(Node node, OFMessage ofm) {
