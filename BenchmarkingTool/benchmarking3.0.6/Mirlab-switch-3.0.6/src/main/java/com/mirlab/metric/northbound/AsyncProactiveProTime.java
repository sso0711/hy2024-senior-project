...
public class AsyncProactiveProTime {
	...
	public static void post() throws Exception {
		...
		AsyncProactiveProTimeThread PPTT = new AsyncProactiveProTimeThread();
		PPTT.start();
...
//			s.setContentEncoding(new BasicHeader(HTTP.CONTENT_TYPE, "application/json"));     // it is reason that bad request?
			...
			httpAsyncClient .execute(request, new FutureCallback<HttpResponse>() {
				...
				public void completed(final HttpResponse response) {
					...
					count = count + 1;
					System.out.println("successful response" + count);	
				//	long ppt=AsyncProactiveProTimeThread.stopTime-AsyncProactiveProTime.startti
