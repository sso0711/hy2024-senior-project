...
public class S_RunButton {
	...
	public static void startTasks() {// 按 run按钮之后的主要实行代码
		// start Metric
		Initializer.INITIAL_CHANNEL_POOL();
		Main.S_LOG_TEXT.setText("");
		Main.S_TOPO_TEXT.setText("");
		...
		switch (Global.TOPO_TYPE) {
		case Global.TOPO_LINEAR:
			Main.S_TOPO_TEXT.append("Topology Type: Linear\n");
			break;
		case Global.TOPO_RING:
			Main.S_TOPO_TEXT.append("Topology Type: Ring\n");
			break;
		case Global.TOPO_MININET:
			Main.S_TOPO_TEXT.append("Topology Type: Mininet\n");
		}
		Main.S_TOPO_TEXT.append("Number Of Node: " + Global.NUMBER_OF_TEST_SWITCH);
		...
		case Global.TEST_TOTAL:
			...
			Global.TEST_INIT_TIME = System.currentTimeMillis();
			Main.S_progressBarTotal.setMaximum(10);
			Main.S_progressBarTotal.setValue(0);
			TestTotal.go();
			break;
		default:
