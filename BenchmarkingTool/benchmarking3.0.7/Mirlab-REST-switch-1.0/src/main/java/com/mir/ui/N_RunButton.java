...
import com.mirlab.metric.northbound.AsyncClientProactivePathProvisioningTime;
import com.mirlab.metric.northbound.AsyncProactiveProvisioningTime;
	...
	public static void startTasks() {// 按 run按钮之后的主要实行代码
		...
		case PROACTIVE_PATH_PROVISIONING:
			...
			try {
				...
				Global.northORwrap = NorthboundMetric.NORTH;
				
				if (Global.sendType == NorthboundMetric.SERIALIZE) {
					Log.N_ADD_LOG_PANEL("### Serialize Provisioning Time", N_RunButton.class.getSimpleName());
					SsyncProactiveProTime.go();
				} else {
					Log.N_ADD_LOG_PANEL("### Pipeline Provisioning Time,Rate", N_RunButton.class.getSimpleName());
					
					AsyncProactiveProvisioningTime.go();
			...
			// 未完待续
			break;
		case SYNC_PROACTIVE_PATH_PROVISIONING_TIME:
