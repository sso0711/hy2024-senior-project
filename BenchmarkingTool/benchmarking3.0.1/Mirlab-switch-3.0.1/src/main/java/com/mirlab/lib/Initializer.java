...
public class Initializer {
	...
	public static void INITIAL_CHANNEL_POOL() {
		...
		ConnectionThread.bootstrap.shutdown();
		...
		// for distributed port

		for (int i = 0; i < Global.distributedPortList.size(); i++) {
			Global.distributedPortList.get(i).closeServer();
		}
		...
		Main.S_startButton.setEnabled(true);
		Main.S_metricList.setEnabled(true);
		Global.PROGRESS_MAX_VALUE = 0;
		Global.CONTROLLER_IS_OPEN = false;

		ConnectionThread.bootstrap = new ClientBootstrap(new NioClientSocketChannelFactory(
	...
	public static void TEST_CONTROLLER_IF_IT_IS_OPEN() {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				ConnectionThread ct = new ConnectionThread(rootNode, Global.SDN_CONTROLLER_IP[0],
	...
	private static void creatTopoOneNode() {
		Node rootNode;
		if (Global.TEST_METRIC == Global.ASYNCHRONOUS_MESSAGE_PROCESSING_TIME) {
			rootNode = new Node((long) (1 + Global.SWITCH_ID_OFF_SET), 0);
		} else if (Global.TEST_METRIC == Global.ASYNCHRONOUS_MESSAGE_PROCESSING_RATE) {
			rootNode = new Node((long) (2 + Global.SWITCH_ID_OFF_SET), 0);
		} else if (Global.TEST_METRIC == Global.REACTIVE_PATH_PROVISIONING_TIME) {
			rootNode = new Node((long) (3 + Global.SWITCH_ID_OFF_SET), 0);
		} else if (Global.TEST_METRIC == Global.REACTIVE_PATH_PROVISIONING_RATE) {
		...
		ConnectionThread ct = new ConnectionThread(rootNode, Global.SDN_CONTROLLER_IP[0], Global.SDN_CONTROLLER_PORT);
		ct.start();
	...
	private static void creatTopoWithoutHost() {
		...
		for (int i = 1; i < Global.NUMBER_OF_TEST_SWITCH; i++) {
			...
			ConnectionThread ct = new ConnectionThread(node, Global.SDN_CONTROLLER_IP[0], Global.SDN_CONTROLLER_PORT);
			ct.start();
		...
		if (Global.TOPO_TYPE == Global.TOPO_RING) {
